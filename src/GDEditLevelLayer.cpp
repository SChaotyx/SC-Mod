...
bool GDEditLevelLayer::Init(GJGameLevel* level) {
    ...
    if(m_pLevel->m_nLevelID){
        auto infoBtn = CCMenuItemSpriteExtra::create(CCSprite::createWithSpriteFrameName("GJ_infoBtn_001.png"), this, (SEL_MenuHandler)(&GDEditLevelLayer::levelInfoLayer));
        infoBtn->setPosition(winSize.width - 120, winSize.height - 170);
        infoBtn->setScale(0.6f);
        infoBtn->m_fBaseScale = 0.6f;
        menu->addChild(infoBtn);

        auto ldbrBtn = CCMenuItemSpriteExtra::create(CCSprite::createWithSpriteFrameName("GJ_levelLeaderboardBtn_001.png"), this, (SEL_MenuHandler)(&GDEditLevelLayer::levelLeaderboardLayer));
